"use client";

import AddTodo from "@/components/Dashboard/Todos/AddTodo";
import TodoList from "@/components/Dashboard/Todos/TodoList";
import UpdateTodo from "@/components/Dashboard/Todos/UpdateTodo";
import { PencilSquareIcon } from "@heroicons/react/24/outline";
import { useState } from "react";

const MainPage = () => {
  const [toBeUpdatedTodoId, setToBeUpdatedTodoId] = useState("");

  const [showAddTodoModal, setShowAddTodoModal] = useState(false);
  const [showUpdateModal, setShowUpdateModal] = useState(false);

  // this is the date to control todos page display, when this changes the correspond todos will appear
  const [date, setDate] = useState(new Date());

  // to refetch data each time adding new todo
  const [refreshTodos, setRefreshTodos] = useState(false); // Trigger re-fetch

  const handleTodoAdded = () => {
    setRefreshTodos((prev) => !prev);
    setShowAddTodoModal(false);
  };

  const handleTodoDeleted = () => {
    setRefreshTodos((prev) => !prev);
  };

  const handleTodoUpdated = () => {
    setRefreshTodos((prev) => !prev);
  };

  const handleShowUpdateModal = () => {
    setShowUpdateModal((prev) => !prev);
    console.log("to be updated todo id: " + toBeUpdatedTodoId);
    console.log("show update modal: " + showUpdateModal);
  };

  return (
    <main className="mb-8 flex flex-col md:p-6  md:py-4">
      {/* add todo modal is shown when the state isModalOpen is true */}
      <div className="relative flex justify-center">
        {showAddTodoModal && (
          <AddTodo
            onClose={() => setShowAddTodoModal(false)}
            onTodoAdded={handleTodoAdded}
            setDate={setDate}
          />
        )}
      </div>

      <div className="relative flex justify-center">
        {showUpdateModal && (
          <UpdateTodo
            toBeUpdatedTodoId={toBeUpdatedTodoId}
            onClose={() => setShowUpdateModal(false)}
            onTodoUpdated={handleTodoUpdated}
            setDate={setDate}
          />
        )}
      </div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <PencilSquareIcon className="w-6 text-gray-700" />
          <h1 className="text-gray-700 font-semibold text-2xl leading-none">
            To-Dos
          </h1>
        </div>
        {/* Add New Todo Button */}
        <button
          className="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-5 rounded-md text-sm transition-all ease-in-out"
          onClick={() => setShowAddTodoModal(true)}
        >
          Add Todo +
        </button>
      </div>

      <div className="h-full">
        <div className="w-full flex justify-center">
          <div className="w-full">
            <TodoList
              date={date}
              setDate={setDate}
              refreshTrigger={refreshTodos}
              onTodoDeleted={handleTodoDeleted}
              showUpdateModal={handleShowUpdateModal}
              setToBeUpdatedTodoId={setToBeUpdatedTodoId}
            />
          </div>
        </div>
      </div>
    </main>
  );
};

export default MainPage;
